...
struct NQueensTests {
    ...
    struct PlacingAQueen {
        ...
        @Test func placesQueenOnEmptySquare() {
            ...
            _ = game.squareTapped(at: 0, on: &board)
        ...
        @Test func doesNotPlaceQueenWhenBoardIsFull() {
            ...
            [0, 5, 10, 15].forEach { _ = game.squareTapped(at: $0, on: &board) }
            // Attempt to place a 5th
            _ = game.squareTapped(at: 1, on: &board)
        ...
        @Test func doesNotPlaceQueenBeyondBoardSize() {
            ...
            [0, 3].forEach { _ = game.squareTapped(at: $0, on: &board) }
            _ = game.squareTapped(at: 1, on: &board)
            let queenCount = board.squares.filter { $0.piece?.type == .queen }.count
            #expect(queenCount == 2)
        }
    ...
    struct RemovingAQueen {
        ...
        @Test func allowsPlacingNewQueenAfterRemoval() {
            var board = Chess.Board(size: 4)
            [0, 5, 10, 15].forEach { _ = game.squareTapped(at: $0, on: &board) }
            _ = game.squareTapped(at: 0, on: &board) // remove
            _ = game.squareTapped(at: 1, on: &board) // should be allowed now
    ...
    struct Progress {
        ...
        @Test func progressStepIncrementsOnPlace() {
            var board = Chess.Board(size: 4)
            let result = game.squareTapped(at: 0, on: &board)
        ...
        @Test func progressStepDecrementsOnRemove() {
            var board = Chess.Board(size: 4)
            _ = game.squareTapped(at: 0, on: &board)
            _ = game.squareTapped(at: 5, on: &board)
            let result = game.squareTapped(at: 0, on: &board) // remove
            #expect(result.progress.step == 1)
        }

        @Test func progressStepIsZeroOnEmptyBoard() {
            var board = Chess.Board(size: 4)
            _ = game.squareTapped(at: 0, on: &board) // add
            let result = game.squareTapped(at: 0, on: &board) // remove
    ...
    struct GameStatus {
        ...
        @Test func statusIsConflictingWhenQueensShareRow() {
            var board = Chess.Board(size: 4)
            _ = game.squareTapped(at: 0, on: &board) // row 0, col 0
            let result = game.squareTapped(at: 1, on: &board) // row 0, col 1
        ...
        @Test func statusIsConflictingWhenQueensShareColumn() {
            var board = Chess.Board(size: 4)
            _ = game.squareTapped(at: 0, on: &board) // row 0, col 0
            let result = game.squareTapped(at: 4, on: &board) // row 1, col 0
            #expect(result.gameStatus == .conflicting)
        }

        @Test func statusIsConflictingWhenQueensSharePositiveDiagonal() {
            var board = Chess.Board(size: 4)
            _ = game.squareTapped(at: 1, on: &board) // row 0, col 1 — diag 1
            let result = game.squareTapped(at: 4, on: &board) // row 1, col 0 — diag 1
            #expect(result.gameStatus == .conflicting)
        }

        @Test func statusIsConflictingWhenQueensShareNegativeDiagonal() {
            var board = Chess.Board(size: 4)
            _ = game.squareTapped(at: 0, on: &board) // row 0, col 0 — diag 0
            let result = game.squareTapped(at: 5, on: &board) // row 1, col 1 — diag 0
            #expect(result.gameStatus == .conflicting)
        }
        ...
        @Test func statusIsSolvedForKnown4x4Solution() {
            ...
            _ = game.squareTapped(at: 1, on: &board)  // row 0, col 1
            _ = game.squareTapped(at: 7, on: &board)  // row 1, col 3
            _ = game.squareTapped(at: 8, on: &board)  // row 2, col 0
            let result = game.squareTapped(at: 14, on: &board) // row 3, col 2
        ...
        @Test func statusIsSolvedForAlternate4x4Solution() {
            ...
            _ = game.squareTapped(at: 2, on: &board)  // row 0, col 2
            _ = game.squareTapped(at: 4, on: &board)  // row 1, col 0
            _ = game.squareTapped(at: 11, on: &board) // row 2, col 3
            let result = game.squareTapped(at: 13, on: &board) // row 3, col 1
            #expect(result.gameStatus == .solved)
        }
        ...
        @Test func statusReturnsToNormalAfterRemovingConflictingQueen() {
            var board = Chess.Board(size: 4)
            _ = game.squareTapped(at: 0, on: &board)
            _ = game.squareTapped(at: 1, on: &board) // conflict
            let result = game.squareTapped(at: 1, on: &board) // remove conflicting queen
            #expect(result.gameStatus == .normal)
        }
    }
    ...
    struct Conflicts {
        ...
        @Test func conflictingSquaresAreMarked() {
            var board = Chess.Board(size: 4)
            _ = game.squareTapped(at: 0, on: &board) // row 0, col 0
            _ = game.squareTapped(at: 1, on: &board) // row 0, col 1 — same row
        ...
        @Test func conflictsAreClearedAfterRemovingConflictingQueen() {
            ...
            _ = game.squareTapped(at: 0, on: &board)
            _ = game.squareTapped(at: 1, on: &board) // conflict
            _ = game.squareTapped(at: 1, on: &board) // remove
    ...
    struct Reset {
        ...
        @Test func removesAllConflicts() {
            var board = Chess.Board(size: 4)
            _ = game.squareTapped(at: 0, on: &board)
            _ = game.squareTapped(at: 1, on: &board) // create conflict
            _ = game.reset(board: &board)
            #expect(board.squares.allSatisfy { !$0.hasConflict })
        }
    ...
    struct Performance {
        ...
        @Test func evaluationOnLargeBoardCompletesWithinTimeLimit() {
            ...
            indices.forEach { _ = game.squareTapped(at: $0, on: &board) }
            
            let clock = ContinuousClock()
            let elapsed = clock.measure {
                for _ in 0..<100 {
                    _ = game.squareTapped(at: indices[0], on: &board)
                }
            }
